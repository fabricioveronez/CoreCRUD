...
namespace CoreCRUD.Api.Validators
{
    ...
    public class ProdutoViewModelValidator : AbstractValidator<ProdutoViewModel>
    {
        ...
        public ProdutoViewModelValidator()
        {
            ...
            RuleFor(produto => produto.Categoria).NotNull().NotEmpty().WithMessage("Produto deve ter uma categoria.");
            // Valida a descrição 
            RuleFor(produto => produto.Descricao).NotNull().NotEmpty().WithMessage("A descrição deve ter no mínimo 20 caracteres.");
        }
    }
}
